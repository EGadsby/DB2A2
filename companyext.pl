...
female_q1(X) :- female(X), works_on(X, computerization, 10), superior(jennifer, X).

employee_40k_q2(X) :- salary(X, Y), Y >= 40000, department(X, research).

supreme_chief_q3(X) :- employee(X), \+ subordinate(X,_).

product_x_e20_q4(X) :- works_on(X, Y, Z), Y = productx, Z >= 20.
